...
static int battery_data_get(void)
{
	 ...
	 */
	uint16_t voltage = 0;
	if (CONFIG_ADC_BATTERY){
		err = battery_sample();
		if (err < 0){
			LOG_ERR("get_battery_voltage, error: %d", err);
			return err;
		}
		voltage = (uint16_t)err;
	}
	else{
		err = modem_info_params_get(&modem_param);
		if (err) {
		...
		voltage = modem_param.device.battery.value;
	}
	...
	modem_module_event->data.bat.battery_voltage = voltage;
...
static int setup(void)
{
	...
	if (CONFIG_ADC_BATTERY == true){
		err = battery_measure_enable(true);
		if (err){
			LOG_ERR("battery_measure_enable, error: %d", err);
			return err;
		}
	}
