...
static void inactive_send(void)
{
	#if defined(CONFIG_LOCATION_METHOD_WIFI) && defined(CONFIG_WIFI_SLEEP)
	esp_sleep_sleep();
	#endif
...
#if defined(CONFIG_WIFI_SLEEP)
	err = esp_sleep_init_device();
	if (err){
		LOG_ERR("Sleep configuration for esp module failed: %d", err);
		return -1;
	}
#endif
	return 0;
